...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  if(flush_tlb) lcr3(V2P(myproc()->pgdir));
  return d;
...
int
handlepagefault(uint addr)
{
  struct proc *p = myproc();
  pte_t *pte = walkpgdir(p->pgdir, (const void *)addr, 0);

  if(!pte || !(*pte & PTE_P)) {
    return allocmem(addr);
  }
  else if (*pte & PTE_COW) {
    uint pa = PTE_ADDR(*pte);
    acquire(&pgref.lock);
    if(pgref.cnt[pa / PGSIZE] > 1){
      pgref.cnt[pa / PGSIZE]--;
      char *mem = kalloc();
      if (mem == 0) panic("Segmentation fault!\n");
